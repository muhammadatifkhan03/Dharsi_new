...
const WritePage = () => {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/posts', {
      ...
      body: JSON.stringify({
        ...
        img: media,
        slug: slugify(title),
        catSlugTitle: catSlug || 'Life Style',
        catSlug: convertToSnakeCase(catSlug) || 'life_style',
      }),
    });
  ...
  const convertToSnakeCase = (inputString) => {
    ...
    // Convert each word to snake_case format
    const snakeCaseWords = words.map((word) => {
      // Capitalize the first letter of each word
      const capitalizedWord = word.charAt(0).toUpperCase() + word.slice(1);
      // Replace spaces with underscores and convert to lowercase
      return capitalizedWord.replace(/\s+/g, '_').toLowerCase();
    });
